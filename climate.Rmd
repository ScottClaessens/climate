...
parameterEstimates(semModel2.1h)[11:15,] %>%
  mutate_if(is.numeric, round, 2) %>%
  kable() %>% kable_styling(full_width = FALSE)
...
# save
fig3b <- fig3b + theme(axis.text.y = element_blank(),
                       axis.ticks.y = element_blank())
fig3 <- plot_grid(fig3a, fig3b, labels = c("a","b"), nrow = 1, rel_widths = c(1, 0.875))
ggsave("figures/fig3.pdf", fig3, width = 7.5, height = 4)
...
```{r echo=F}
fitMeasures(semModel4.1a)["rmsea"] %>% round(3)
fitMeasures(semModel4.1a)["srmr"] %>% round(3)
```
...
Here we calculate understandardised estimates.
...
These coefficients suggest a *partial* mediation effect. Including environmental sacrifice as a mediator decreases the unstandardised parameter from the direct path. However, it remains positive and significant. The indirect path is also positive and significant. So some, but not all, of the effect of cooperative phenotype on climate change belief is explained by the mediation.

In the code (hidden here), we show that this partial mediation effect remains after controlling for all factors, except political party support. When including party support, the path from the cooperative phenotype to climate change belief is attenuated.
...
How much variation does the full model account for?
...
```{r echo=F}
fitMeasures(semModel4.1h)["rmsea"] %>% round(3)
fitMeasures(semModel4.1h)["srmr"] %>% round(3)
```
...
This means that this model accounts for 20% of the variation in sacrifice enacted, and 3% of the variation in climate change belief.
...
node_set <- 
  combine_ndfs(latent, manifest) %>%
  mutate(x = c(3, 4, 2, 2, 2, 2, 2, 5, 3.25, 4, 4.75, 1, 1, 1, 1),
         y = c(3, 4, 1.4, 2.2, 3, 3.8, 4.6, 3, 5, 5, 5, 1.4, 2.6, 3.8, 4.6))
...
This is a full mediation effect. In the code (hidden here), We show that this full mediation effect remains after controlling for all factors except political party support.
...
model <- '# measurement model
          ...
          # regression
          ccb ~ coop + age
          sacrifice ~ ccb + coop + age'
semModel4.1i <- sem(model, data = d_temp, ordered = c('tg1','sh','real',
                                                      'human','concern','sacrifice'))
# parameterEstimates(semModel4.1i)[14:18,]
...
model <- '# measurement model
          ...
          # regression
          ccb ~ coop + gender
          sacrifice ~ ccb + coop + gender'
semModel4.1j <- sem(model, data = d_temp, ordered = c('tg1','sh','real',
                                                      'human','concern','sacrifice'))
# parameterEstimates(semModel4.1j)[14:18,]
...
model <- '# measurement model
          ...
          # regression
          ccb ~ coop + ethnic_Asian + ethnic_Maori + ethnic_Pacific
          sacrifice ~ ccb + coop  + ethnic_Asian + ethnic_Maori + ethnic_Pacific'
semModel4.1k <- sem(model, data = d_temp, ordered = c('tg1','sh','real',
                                                      'human','concern','sacrifice'))
# parameterEstimates(semModel4.1k)[14:22,]
...
model <- '# measurement model
          ...
          # regression
          ccb ~ coop + party_Labour + party_Greens + party_National + party_NZFirst + party_Other
          sacrifice ~ ccb + coop + party_Labour + party_Greens + party_National + party_NZFirst + party_Other'
semModel4.1l <- sem(model, data = d_temp, ordered = c('tg1','sh','real',
                                                      'human','concern','sacrifice'))
# parameterEstimates(semModel4.1l)[14:26,]
...
model <- '# measurement model
          ...
          # regression
          ccb ~ coop + nzreg
          sacrifice ~ ccb + coop + nzreg'
semModel4.1m <- sem(model, data = d_temp, ordered = c('tg1','sh','real',
                                                      'human','concern','sacrifice'))
# parameterEstimates(semModel4.1m)[14:18,]
...
model <- '# measurement model
          ...
          # regression
          ccb ~ coop + age + gender + ethnic_Asian + ethnic_Maori + ethnic_Pacific + party_Labour + party_Greens + party_National + party_NZFirst + party_Other + nzreg
          sacrifice ~ ccb + coop + age + gender + ethnic_Asian + ethnic_Maori + ethnic_Pacific + party_Labour + party_Greens + party_National + party_NZFirst + party_Other + nzreg'
semModel4.1n <- sem(model, data = d_temp, ordered = c('tg1','sh','real',
                                                      'human','concern','sacrifice'))
# parameterEstimates(semModel4.1n)[14:38,]
```
