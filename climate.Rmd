---
title: "Climate Change & Cooperative Phenotype - Analyses controlling for comprehension"
...
The purpose of this document is to report the further analyses for the climate change project, controlling for comprehension.
...
fig01 <- 
  arrangeGrob(fig01a, fig01b,
               fig01c, fig01d, layout_matrix = layout)
fig01 <- arrangeGrob(fig01, left = y.grob)

# save for publication
ggsave("figures/fig01.pdf", fig01, width = 22, height = 15, units = "cm")
ggsave("figures/fig01.png", fig01, width = 22, height = 15, units = "cm")
...
This relationship is easier to see if we directly plot these predictions. Below, the regression line is the change in mean score of environmental sacrifice enacted (1-7) with each unit change in the cooperation factor.
...
fig2a <-
  ...
  coord_flip() +
  xlab(NULL) +
  ylab("Unstandardised Estimate\n(coop phenotype predicting pro-env behaviour)") +
  ylim(-0.7,1.7)
...
fig3a <-
  ...
  ggplot(aes(x = sacrifice, y = cut, group= sacrifice)) +
  geom_boxplot(outlier.shape = NA) +
  geom_jitter(width = 0.2, height = 0.3, colour = "darkseagreen4", alpha = 0.3) +
  coord_flip() +
  scale_x_continuous(breaks = 1:7) +
  scale_y_discrete(labels = seq(0, 100, length.out = 11)) +
  labs(y = "Contribution in\nPublic Goods Game (points)", x = "Have you made sacrifices to your\nstandard of living in order to\nprotect the environment? (1-7)") +
  theme(axis.title.y = element_text(size = 11))
...
This means that the latent variable "cooperative phenotype" accounts for 3% of the variation in the latent varaible "climate change beliefs".
...
Plotting will again help with interpretation.
...
# save
fig5 <- plot_grid(fig5a, fig5b + theme(axis.title.y = element_blank()), 
                  labels = c("a","b"), nrow = 1, align = "vh")
ggsave("figures/fig5.pdf", fig5, width = 9, height = 4)
ggsave("figures/fig5.png", fig5, width = 9, height = 4)

rm(fig5a, fig5b)
```

This shows that political party support is responsable for the majority of the increase in variation in climate change beliefs explained by the full model. Below, we investigate how the slope of cooperative phenotype (the unstandardised parameter) varies across these models.
...
fig2b <-
  ...
  coord_flip() +
  xlab(NULL) +
  ylab("Unstandardised Estimate\n(coop phenotype predicting climate change belief)") +
  ylim(-0.8,1.9) +
...
# save
fig2 <- plot_grid(fig2a, fig2b, labels = c("a","b"), nrow = 1, align = "vh")
ggsave("figures/fig2.pdf", fig2, width = 9, height = 4)
ggsave("figures/fig2.png", fig2, width = 9, height = 4)

rm(fig2a, fig2b, estimate, se, i, out)
```
...
fig3b <-
  d %>%
  mutate(cut = cut(pgg*100, 11)) %>%
  ggplot(aes(x = cut, y = ccb_semModel3.1a)) +
  geom_boxplot(outlier.shape = NA) +
  geom_jitter(aes(x = (pgg*10)+1), height = 0.3, width = 0.2, colour = "darkseagreen4", alpha = 0.3) +
  labs(x = "Contribution in\nPublic Goods Game (points)", y = "Climate Change Belief") +
  scale_x_discrete(labels = seq(0, 100, length.out = 11))

fig3b

# save 
fig3 <- plot_grid(fig3a + theme(axis.title.x = element_blank()), 
                  fig3b, labels = c("a","b"), ncol = 1, align = "v")
ggsave("figures/fig3.pdf", fig3, width = 5, height = 6)
ggsave("figures/fig3.png", fig3, width = 5, height = 6)
...
This means that the full model accounts for 28% of the variance in the latent variable "climate change beliefs".
...
# save
fig4 <- plot_grid(fig4a, fig4b, labels = c("a","b"), nrow = 1, align = "vh")
ggsave("figures/fig4.pdf", fig4, width = 9, height = 4)
ggsave("figures/fig4.png", fig4, width = 9, height = 4)

rm(fig4a, fig4b, estimate, se, i, out)
```
...
This model has a little trouble fitting, likely because of its complexity.
...
```{r echo=F, message=F, warning=F}
# publication quality path diagram
...
# save to file
export_graph(fig6, file_name = "figures/fig6.pdf")
export_graph(fig6, file_name = "figures/fig6.png")

# cleanup
rm(all_paths, edge_set, latent, loadings, manifest, 
   node_set, paths, regressions, nodes)
```
...
```{r}
sqrt(inspect(semModel4.1a, "r2"))
```
The effect size of cooperative phenotype on enviornmental sacrifice is small (r = 0.10), and the combined effect of cooperative phenotype and environmental sacrifice on climate change belief is moderate (0.46). 
...
d_temp <- d %>% drop_na(age)
model <- 'sacrifice ~ age'
semModel2.1.1a <- sem(model, data = d_temp, ordered = c('sacrifice'))

# gender (n = 896)
d_temp <- d %>% drop_na(gender)
model <- 'sacrifice ~ gender'
semModel2.1.1b <- sem(model, data = d_temp, ordered = c('sacrifice'))

# ethnicity (n = 890)
d_temp <- d %>% drop_na(ethnic)
model <- 'sacrifice ~ ethnic_Asian + ethnic_Maori + ethnic_Pacific'
semModel2.1.1c <- sem(model, data = d_temp, ordered = c('sacrifice'))

# party support (n = 877)
d_temp <- d %>% drop_na(party)
model <- 'sacrifice ~ party_Labour + party_Greens + party_National + party_NZFirst + party_Other'
semModel2.1.1d <- sem(model, data = d_temp, ordered = c('sacrifice'))

# nzreg (n = 896)
d_temp <- d %>% drop_na(nzreg)
model <- 'sacrifice ~ nzreg'
semModel2.1.1e <- sem(model, data = d_temp, ordered = c('sacrifice'))

#building the graph
fig5c <-  
  tibble(
    model = c("coop","age","gender","ethn", "party", "edu", "full"),
    r2 = c(inspect(semModel2.1a,"r2")[6],
           inspect(semModel2.1.1a, "r2"),
           inspect(semModel2.1.1b, "r2"),
           inspect(semModel2.1.1c, "r2"),
           inspect(semModel2.1.1d, "r2"),
           inspect(semModel2.1.1e, "r2"),
...
This shows that cooperative phenotype explains a similar amount of the variance in pro-environmental behaviour as ethnicity, and education. Interestingly, it explains more vairance than age.

Below, we investigate the same comparisons with our latent climate change belief variable.
...
model <- '# measurement model
          ...
          # regression
          ccb ~ age'
semModel3.1.1a <- sem(model, data = d_temp, ordered = c('real','human','concern'))
...
model <- '# measurement model
          ...
          # regression
          ccb ~ gender'
semModel3.1.1b <- sem(model, data = d_temp, ordered = c('real','human','concern'))
...
semModel3.1.1c <- sem(model, data = d_temp, ordered = c('real','human','concern'))
...
semModel3.1.1d <- sem(model, data = d_temp, ordered = c('real','human','concern'))
...
model <- '# measurement model
          ...
          # regression
          ccb ~ nzreg'
semModel3.1.1e <- sem(model, data = d_temp, ordered = c('real','human','concern'))
...
fig5d <-  
  tibble(
    model = c("coop","age","gender","ethn", "party", "edu", "full"),
    r2 = c(inspect(semModel3.1a,"r2")[9],
           inspect(semModel3.1.1a, "r2")[4],
           inspect(semModel3.1.1b, "r2")[4],
           inspect(semModel3.1.1c, "r2")[4],
           inspect(semModel3.1.1d, "r2")[4],
           inspect(semModel3.1.1e, "r2")[4],
           inspect(semModel3.1g,"r2")[9])
  ) %>% 
  ggplot(aes(x = factor(model, levels = .$model), y = r2)) +
  geom_bar(stat = "identity", fill = "skyblue4") +
  scale_y_continuous(limits = c(0, 0.3)) +
  labs(y = bquote(R^2), x = "climate change belief")

fig5d 

# save
fig5_1 <- plot_grid(fig5c, fig5d + theme(axis.title.y = element_blank()), 
                  labels = c("a","b"), nrow = 1, align = "vh")
ggsave("figures/fig5_1.pdf", fig5_1, width = 9, height = 4)
ggsave("figures/fig5_1.png", fig5_1, width = 9, height = 4)

rm(fig5c, fig5d)
```

This shows that age accounts for 3% of the varation in climate change belief, and that gender accounts for 1% of the variation in climate change belief. Similarly, ethnicity, and education account for 1% and 8% of the variation in climate change belief respectively, while party accounts for 24%. This is comparable to the respective variance in climate change belief explained by cooperative phenotype alone (3%). 
...
# save
fig8 <- plot_grid(fig8a, fig8b, fig8c, fig8d,
                  labels = c("a","b","c","d"), 
                  nrow = 2, ncol = 2, align = "vh")
ggsave("figures/fig8.pdf", fig8, width = 8, height = 6)
ggsave("figures/fig8.png", fig8, width = 8, height = 6)

# cleanup
rm(fig8a, fig8b, fig8c, fig8d)
```
